...
(* 
  Excluding these two functions (compute_peak_all_players and add_peak_data_to_db) from test coverage. 
  compute_peak_all_players will take way too long on the full dataset.
  add_peak_data_to_db is a simple database operation and isn't worthy of testing.
*)
[@@@coverage off]
let compute_peak_all_players (peak_size: int) : Dataframe.t = 
  ...
  let compute_peak (row: Dataframe.elt array) : unit = 
  ...
  in 
  Dataframe.iter_row compute_peak players; 
  output_df
