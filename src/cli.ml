...
let menu_choice_loop (print_menu: unit -> unit) (choices: int list) (selection_handler: int -> bool ref -> unit) = 
  print_menu ();
...
let perform_main_menu_selection (choice: int) (quit: bool ref) = 
  ...
  | 42 -> menu_choice_loop print_db_menu db_admin_menu_choices perform_db_menu_selection
  | _ -> failwith "Unreachable case: user menu choice should already be validated at this point."
